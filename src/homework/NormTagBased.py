        ...
        for tag, wut in self.user_tags[user].items():
            ...
            for item, wti in self.tag_items[tag].items():
                ...
                if item not in recommend_items:
                    recommend_items[item] = (wut / sum(self.user_tags[user].values())) * (wti / sum(self.tag_items[tag].values()))*10000
                else:
                    recommend_items[item] += (wut / sum(self.user_tags[user].values())) * (wti / sum(self.tag_items[tag].values()))*10000
...
if __name__=='__main__':
    stb=NormTagBased("E:/Spark学习/RS_Study/src/teacher/user_taggedbookmarks-timestamps.dat")